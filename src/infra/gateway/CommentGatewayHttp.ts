...
export default class CommentGatewayHttp {
    ...
    async makeAComment(tuneetId: string, authorId: string, contentText: string): Promise<any> {
        try {
            return await this.httpClient.post(`${this.url}`, { authorId, tuneetId, contentText });
        } catch (error: any) {
