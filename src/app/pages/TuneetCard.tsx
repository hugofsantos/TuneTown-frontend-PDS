          ...
          <div className="flex  flex-1 flex-col gap-4 ">
            {comments.length === 0 ? (
              ...
              comments.map((comment) => (
                <div
                  key={comment.id}
                  className="flex flex-col gap-4 bg-black/10 rounded p-2"
                >
                  <span className="text-xs flex justify-between text-theme font-bold">
                    <p>@{comment.authorUsername}</p>
                    <p>{new Date(comment?.createdAt).toLocaleString()}</p>
                  </span>
                  <span className="text-xs text-contrast/50 flex justify-between">
                    {" "}
                    {comment.contentText}
                  </span>
